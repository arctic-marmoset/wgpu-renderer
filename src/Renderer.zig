...
pub fn loadModel(
    ...
    for (gltf.data.images.items) |image| {
        if (image.uri) |uri| {
            ...
            const texture = try loadTexture(self.allocator, self.device, self.queue, dir, uri);
            ...
            try self.textures.put(self.allocator, uri_owned, texture);
        }
    }
    ...
    for (gltf.data.materials.items) |material| {
        ...
        const texture_bind_group = blk: {
            if (material.metallic_roughness.base_color_texture) |texture| {
                ...
                const texture_view = self.textures.get(uri) orelse continue;
        ...
        try materials.append(self.allocator, .{
...
fn loadNodes(
    ...
    for (nodes) |node_index| {
        ...
        for (gltf_mesh.primitives.items, 0..) |primitive, i| {
            ...
            log.debug("primitive {}: {?s}", .{
                i,
                if (material_index) |index| gltf.data.materials.items[index].name else null,
            });
            ...
            const position_accessor = maybe_position_accessor orelse continue;
            const normal_accessor = maybe_normal_accessor orelse continue;
            const texcoord_accessor = maybe_texcoord_accessor orelse continue;
            ...
            try primitives.append(self.allocator, .{
        ...
        try meshes.append(self.allocator, .{
